'use strict';
/*
 * Sagu - OAuth 2.1 til claude.ai's connectors.
 *
 * **Porteret naesten ordret fra doda.** Motoren kender hverken database eller
 * http: den faar seks funktioner ind (`gemKlient`, `hentKlient`,
 * `udstedTokens`, `findRefresh`, `tilbagekaldRefresh`) og rører aldrig et
 * `res`. Derfor kunne den flyttes uden at blive skrevet om - og derfor staar
 * det Sagu-specifikke i server.js i stedet.
 *
 * Claude Code og Desktop kan sende en fast noegle i en header. Webklienten
 * kan ikke: den kender ikke din server pa forhaand, saa den skal kunne
 ...
 *
 * Sagu har fire scopes, men `capture` og `link` er lavet til en genvej og til
 * en soesterapp, der kender sin egen opgave. En connector, der forhandler sin
 * rettighed gennem en samtykkeside, skal kunne beskrives i én saetning til
 * den, der trykker »Allow« - og »kan skrive, men ikke laese« er ikke en
 * saetning, nogen traeffer et valg paa. De to smalle scopes laves i haanden
 * under Settings, hvor der staar hvad de kan.
 */
...
function opret(srv) {
  ...
  function registrer(krop) {
    ...
    const id = `sagu-client-${crypto.randomBytes(12).toString('hex')}`;
    srv.gemKlient({
      id,
